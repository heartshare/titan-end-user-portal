...
public class VMController {
	...
	public String main(ModelMap model, String vmName) {
		String error = null;
		try {
			...
			for (int x = 0; x < servers.size(); x++) {
				...
				try {
					...
					ht.put("flavorName", flavors.get(flavorId).get("name"));
					ht.put("flavorRam", flavors.get(flavorId).get("ram"));
					ht.put("flavorVcpus", flavors.get(flavorId).get("vcpus"));
					ht.put("flavorDisk", flavors.get(flavorId).get("disk"));

					if (vmName == null || name.toLowerCase().contains(vmName.toLowerCase())) {
						instances.add(ht);
					}
				} catch (Exception ex) {
					ex.printStackTrace();
